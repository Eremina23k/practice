...
def creat_comp(comp: CompetitionIn, db: Session = Depends(get_db)):
    return creat_competition(db, comp)

@router.get("/{competition_id}", response_model=CompetitionOut)
def get_comp(competition_id: int, db: Session = Depends(get_db)):
...
def put_comp(competition_id: int, comp: CompetitionIn, db: Session = Depends(get_db)):
